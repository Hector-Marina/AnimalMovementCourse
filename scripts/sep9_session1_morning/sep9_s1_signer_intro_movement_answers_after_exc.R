...
# Use the `mutate()` function to add a new column to
...
# ... S: Solution -----
animal1
animal1 |> mutate(month = month(t_, label = TRUE)) 

animal1$month <- month(animal1$t_, label = TRUE)
...
# different bursts.
animal2
table(animal2$burst_)

# ... E: Exercise - resampling ----
# Resample the animal1 on a 4h sampling rate, with a tolerance of 5 minutes and 
...
animal5$data[[1]] %>% mutate(sl = step_lengths(.))
animal5$data[[2]] %>% mutate(sl = step_lengths(.))
animal5$data[[3]] %>% mutate(sl = step_lengths(.))

map(animal5$data, ~ mutate(.x, sl = step_lengths(.)))
