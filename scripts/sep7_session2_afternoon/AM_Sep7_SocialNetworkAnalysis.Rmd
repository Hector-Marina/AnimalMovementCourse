...
```{r message=FALSE}
# Plot the walked areas
{ggplot(FA_data, aes(x, y, color = TagCol)) +
  annotation_custom(Swedish_rg) +
  geom_point(color = FA_data$TagCol) +
  theme_void() +
  theme(legend.position = "none", aspect.ratio = Y_png/X_png) +
  xlim(0,X_png) + ylim(1695,Y_png) +
  transition_time(c(1:86400,1:86400)) +
  ease_aes('linear') +
...
In social network analysis, an egocentric network is a sub network centered around a specific individual, referred to as the "ego." This sub network includes the ego (central individual) along with their directly connected social contacts, known as neighbors. These networks provide insights into the immediate social environment of an individual. Analyzing egocentric networks can provide information about: - *Neighbors degree Centrality*: The number of connections each neighbor has. This gives insight into the social activity and prominence of the ego's contacts. - *Ties among neighbors*: The presence or absence of connections between neighbors. This reveals potential subgroups within the ego's network. - *Structural Holes*: The absence of connections between alters indicates the possibility of the ego transmitting information between disconnected parts of its network. - *Homophily*: Exploring whether the neighbors share certain characteristics with the ego.
